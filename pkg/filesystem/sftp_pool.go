...
// For backward compatibility - calls NewSFTPClientPoolWithLimits with sensible defaults.
func NewSFTPClientPool(sshClient *ssh.Client, maxSize int) (*SFTPClientPool, error) {
	// Use maxSize as both initial and max, with min=1 for backward compatibility
	minSize := 1
	if maxSize < minSize {
		minSize = maxSize
	}
	return NewSFTPClientPoolWithLimits(sshClient, maxSize, minSize, maxSize)
}
...
func NewSFTPClientPoolWithLimits(sshClient *ssh.Client, initialSize, minSize, maxSize int) (*SFTPClientPool, error) {
	// Validate bounds
	if minSize <= 0 {
		return nil, fmt.Errorf("minSize must be greater than 0, got %d", minSize)
	}
	if initialSize < minSize {
		return nil, fmt.Errorf("initialSize (%d) must be >= minSize (%d)", initialSize, minSize)
	}
	if initialSize > maxSize {
		return nil, fmt.Errorf("initialSize (%d) must be <= maxSize (%d)", initialSize, maxSize)
	}
	...
	pool := &SFTPClientPool{
		...
		minSize:    minSize,
		maxSize:    maxSize,
		targetSize: int32(initialSize),
		actualSize: int32(initialSize),
		closed:     false,
	}
...
func (p *SFTPClientPool) Resize(targetSize int) {
	...
	clamped := targetSize
	if clamped < p.minSize {
		clamped = p.minSize
	}
	if clamped > p.maxSize {
		clamped = p.maxSize
	}

	atomic.StoreInt32(&p.targetSize, int32(clamped))
