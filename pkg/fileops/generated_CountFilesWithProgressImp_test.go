...
// CountFilesWithProgressImp wraps a callable function for testing.
// Create with NewCountFilesWithProgressImp(t, yourFunction), call Start() to execute,
// then use ExpectReturnedValuesAre/Should() or ExpectPanicWith() to verify behavior.
type CountFilesWithProgressImp struct {
	*_imptest.CallableController[CountFilesWithProgressImpReturn]
	callable func(rootPath string, progressCallback fileops.CountProgressCallback) (int, error)
}

// NewCountFilesWithProgressImp creates a new wrapper for testing the callable function.
// Pass the function to test and a testing.TB to enable assertion failures.
//
// Example:
//
//	wrapper := NewCountFilesWithProgressImp(t, myFunction)
//	wrapper.Start(args...).ExpectReturnedValuesAre(expectedVals...)
func NewCountFilesWithProgressImp(t _testing.TB, callable func(rootPath string, progressCallback fileops.CountProgressCallback) (int, error)) *CountFilesWithProgressImp {
	return &CountFilesWithProgressImp{
		CallableController: _imptest.NewCallableController[CountFilesWithProgressImpReturn](t),
		callable:           callable,
	}
}
...
func (s *CountFilesWithProgressImp) ExpectPanicWith(expected any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *CountFilesWithProgressImp) ExpectReturnedValuesAre(v1 int, v2 error) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *CountFilesWithProgressImp) ExpectReturnedValuesShould(v1 any, v2 any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *CountFilesWithProgressImp) Start(rootPath string, progressCallback fileops.CountProgressCallback) *CountFilesWithProgressImp {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				s.PanicChan <- r
		...
		ret0, ret1 := s.callable(rootPath, progressCallback)
		s.ReturnChan <- CountFilesWithProgressImpReturn{
			Result0: ret0,
			Result1: ret1,
		}
	}()
...
// CountFilesWithProgressImpResponse represents the response from the callable (either return or panic).
...
type CountFilesWithProgressImpResponse struct {
	EventType string // "return" or "panic"
	ReturnVal *CountFilesWithProgressImpReturn
	PanicVal  any
}

// Type returns the event type: "return" for normal returns, "panic" for panics.
func (r *CountFilesWithProgressImpResponse) Type() string {
	return r.EventType
}

// CountFilesWithProgressImpReturn holds the return values from the callable function.
// Access individual return values via Result0, Result1, etc. fields.
type CountFilesWithProgressImpReturn struct {
	Result0 int
	Result1 error
}
