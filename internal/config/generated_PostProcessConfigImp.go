...
// PostProcessConfigImp wraps a callable function for testing.
// Create with NewPostProcessConfigImp(t, yourFunction), call Start() to execute,
// then use ExpectReturnedValuesAre/Should() or ExpectPanicWith() to verify behavior.
type PostProcessConfigImp struct {
	*_imptest.CallableController[PostProcessConfigImpReturn]
	callable func(cfg *Config) (*Config, error)
}

// NewPostProcessConfigImp creates a new wrapper for testing the callable function.
// Pass the function to test and a testing.TB to enable assertion failures.
//
// Example:
//
//	wrapper := NewPostProcessConfigImp(t, myFunction)
//	wrapper.Start(args...).ExpectReturnedValuesAre(expectedVals...)
func NewPostProcessConfigImp(t _testing.TB, callable func(cfg *Config) (*Config, error)) *PostProcessConfigImp {
	return &PostProcessConfigImp{
		CallableController: _imptest.NewCallableController[PostProcessConfigImpReturn](t),
		callable:           callable,
	}
}
...
func (s *PostProcessConfigImp) ExpectPanicWith(expected any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *PostProcessConfigImp) ExpectReturnedValuesAre(v1 *Config, v2 error) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *PostProcessConfigImp) ExpectReturnedValuesShould(v1 any, v2 any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *PostProcessConfigImp) Start(cfg *Config) *PostProcessConfigImp {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				s.PanicChan <- r
		...
		ret0, ret1 := s.callable(cfg)
		s.ReturnChan <- PostProcessConfigImpReturn{
			Result0: ret0,
			Result1: ret1,
		}
	}()
...
// PostProcessConfigImpResponse represents the response from the callable (either return or panic).
...
type PostProcessConfigImpResponse struct {
	EventType string // "return" or "panic"
	ReturnVal *PostProcessConfigImpReturn
	PanicVal  any
}

// Type returns the event type: "return" for normal returns, "panic" for panics.
func (r *PostProcessConfigImpResponse) Type() string {
	return r.EventType
}

// PostProcessConfigImpReturn holds the return values from the callable function.
// Access individual return values via Result0, Result1, etc. fields.
type PostProcessConfigImpReturn struct {
	Result0 *Config
	Result1 error
}
