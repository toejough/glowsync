...
// FormatBytesImp wraps a callable function for testing.
// Create with NewFormatBytesImp(t, yourFunction), call Start() to execute,
// then use ExpectReturnedValuesAre/Should() or ExpectPanicWith() to verify behavior.
type FormatBytesImp struct {
	*_imptest.CallableController[FormatBytesImpReturn]
	callable func(bytes int64) string
}

// NewFormatBytesImp creates a new wrapper for testing the callable function.
// Pass the function to test and a testing.TB to enable assertion failures.
//
// Example:
//
//	wrapper := NewFormatBytesImp(t, myFunction)
//	wrapper.Start(args...).ExpectReturnedValuesAre(expectedVals...)
func NewFormatBytesImp(t _testing.TB, callable func(bytes int64) string) *FormatBytesImp {
	return &FormatBytesImp{
		CallableController: _imptest.NewCallableController[FormatBytesImpReturn](t),
		callable:           callable,
	}
}
...
func (s *FormatBytesImp) ExpectPanicWith(expected any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *FormatBytesImp) ExpectReturnedValuesAre(v1 string) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *FormatBytesImp) ExpectReturnedValuesShould(v1 any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *FormatBytesImp) Start(bytes int64) *FormatBytesImp {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				s.PanicChan <- r
		...
		ret0 := s.callable(bytes)
		s.ReturnChan <- FormatBytesImpReturn{
			Result0: ret0,
		}
	}()
	return s
...
// FormatBytesImpResponse represents the response from the callable (either return or panic).
...
type FormatBytesImpResponse struct {
	EventType string // "return" or "panic"
	ReturnVal *FormatBytesImpReturn
	PanicVal  any
}

// Type returns the event type: "return" for normal returns, "panic" for panics.
func (r *FormatBytesImpResponse) Type() string {
	return r.EventType
}

// FormatBytesImpReturn holds the return values from the callable function.
// Access individual return values via Result0, Result1, etc. fields.
type FormatBytesImpReturn struct {
	Result0 string
}
