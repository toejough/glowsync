...
// Create with NewEngineEnableFileLogging(t, yourFunction), call Start() to execute,
// then use ExpectReturnedValuesAre/Should() or ExpectPanicWith() to verify behavior.
type EngineEnableFileLogging struct {
	*_imptest.CallableController[EngineEnableFileLoggingReturn]
	callable func(logPath string) error
}

// NewEngineEnableFileLogging creates a new wrapper for testing the callable function.
// Pass the function to test and a testing.TB to enable assertion failures.
//
// Example:
//
//	wrapper := NewEngineEnableFileLogging(t, myFunction)
//	wrapper.Start(args...).ExpectReturnedValuesAre(expectedVals...)
func NewEngineEnableFileLogging(t _testing.TB, callable func(logPath string) error) *EngineEnableFileLogging {
	return &EngineEnableFileLogging{
		CallableController: _imptest.NewCallableController[EngineEnableFileLoggingReturn](t),
		callable:           callable,
	}
}
...
func (s *EngineEnableFileLogging) ExpectPanicWith(expected any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *EngineEnableFileLogging) ExpectReturnedValuesAre(v1 error) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *EngineEnableFileLogging) ExpectReturnedValuesShould(v1 any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *EngineEnableFileLogging) Start(logPath string) *EngineEnableFileLogging {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				s.PanicChan <- r
		...
		ret0 := s.callable(logPath)
		s.ReturnChan <- EngineEnableFileLoggingReturn{
			Result0: ret0,
		}
	}()
	return s
...
// EngineEnableFileLoggingResponse represents the response from the callable (either return or panic).
...
type EngineEnableFileLoggingResponse struct {
	EventType string // "return" or "panic"
	ReturnVal *EngineEnableFileLoggingReturn
	PanicVal  any
}

// Type returns the event type: "return" for normal returns, "panic" for panics.
func (r *EngineEnableFileLoggingResponse) Type() string {
	return r.EventType
}

// EngineEnableFileLoggingReturn holds the return values from the callable function.
// Access individual return values via Result0, Result1, etc. fields.
type EngineEnableFileLoggingReturn struct {
	Result0 error
}
