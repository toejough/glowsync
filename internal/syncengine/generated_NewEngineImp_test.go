...
// NewNewEngineImp creates a new wrapper for testing the callable function.
...
func (s *NewEngineImp) ExpectPanicWith(expected any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *NewEngineImp) ExpectReturnedValuesAre(v1 *syncengine.Engine) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *NewEngineImp) ExpectReturnedValuesShould(v1 any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *NewEngineImp) Start(source, dest string) *NewEngineImp {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				s.PanicChan <- r
		...
		ret0 := s.callable(source, dest)
		s.ReturnChan <- NewEngineImpReturn{
			Result0: ret0,
		}
	}()
	return s
...
// NewEngineImpResponse represents the response from the callable (either return or panic).
...
type NewEngineImpResponse struct {
	EventType string // "return" or "panic"
	ReturnVal *NewEngineImpReturn
	PanicVal  any
}

// Type returns the event type: "return" for normal returns, "panic" for panics.
func (r *NewEngineImpResponse) Type() string {
	return r.EventType
}

// NewEngineImpReturn holds the return values from the callable function.
// Access individual return values via Result0, Result1, etc. fields.
type NewEngineImpReturn struct {
	Result0 *syncengine.Engine
}
