...
// EngineCloseLog wraps a callable function for testing.
// Create with NewEngineCloseLog(t, yourFunction), call Start() to execute,
// then use ExpectReturnedValuesAre/Should() or ExpectPanicWith() to verify behavior.
type EngineCloseLog struct {
	*_imptest.CallableController[struct{}]
	callable func()
}

// NewEngineCloseLog creates a new wrapper for testing the callable function.
// Pass the function to test and a testing.TB to enable assertion failures.
//
// Example:
//
//	wrapper := NewEngineCloseLog(t, myFunction)
//	wrapper.Start(args...).ExpectReturnedValuesAre(expectedVals...)
func NewEngineCloseLog(t _testing.TB, callable func()) *EngineCloseLog {
	return &EngineCloseLog{
...
func (s *EngineCloseLog) ExpectPanicWith(expected any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *EngineCloseLog) ExpectReturnedValuesAre() {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *EngineCloseLog) ExpectReturnedValuesShould() {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *EngineCloseLog) Start() *EngineCloseLog {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				s.PanicChan <- r
...
// EngineCloseLogResponse represents the response from the callable (either return or panic).
